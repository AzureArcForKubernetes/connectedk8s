...
| Event wrapper | `_utils.py :: add_connectedk8s_telemetry_event` | Sanitizes string properties and adds the stored ARM ID to extension event properties |
...
The wrapper copies the supplied dictionary, removes apostrophes from string values
to keep Azure CLI telemetry JSON parseable, and adds:
...
wrapper. Client proxy download, port, startup, configuration, unexpected
closure failures use the `AZK8S0800` through `AZK8S0804` classifications.
The `AZK8S0805` cluster-credential definition is reserved for migration after
the shared ARM error-classification changes land.
