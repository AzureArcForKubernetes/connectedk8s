...
diagnostics use the ARM ID-aware event wrapper. Client proxy download, port,
startup, configuration, and unexpected closure failures use the `AZK8S0800`
through `AZK8S0804` classifications. The `AZK8S0805` cluster-credential
definition is reserved for migration after the shared ARM error-classification
changes land.
